...
class Visualization(HasTraits):
    options = List(
        editor=CheckListEditor(values=["Show Control Mesh", "Show Shadow Path"])
    )
    ...
    def initialize(self, surfaces):
        ...
        for surface in surfaces:
            ...
            if surface.winding_path_file is not None:
                ...
                shadow_plot = mlab.plot3d(*path[:, 7:].T, tube_radius=0.003, color=(1.0, 0.0, 0.0))
                # mlab.points3d(*path[:, 7:].T, scale_factor=0.015)
    ...
    def on_option_change(self):
        ...
        if "Show Shadow Path" in self.options and self.path_type != "None":
            self.paths[int(self.mesh)][-1].visible = True
        else:
            self.paths[int(self.mesh)][-1].visible = False
    ...
    def on_path_type_change(self):
        ...
        self.paths[int(self.mesh)][-1].visible = "Show Shadow Path" in self.options

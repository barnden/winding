...
{
    m_winding_order = decltype(m_winding_order)(m_initial.size());
    std::iota(m_winding_order.begin(), m_winding_order.end(), 0uz);
    ...
    // for (auto i = 1uz; i < m_winding_order.size(); i++) {
    //     if (i % 2 == 1)
    //         continue;

    //     auto const e = m_winding_order[i - 2];

    //     std::cout << e << '\t' << m_initial[e][0].x() << '\t' << fmod(m_initial[e][0].x(), 2. * PI) << '\n';
    // }

    // std::cout << "\n\n";

    // for (auto&& [i, e] : enumerate(m_winding_order)) {
    //     if (i % 2 == 1)
    //         std::cout << e << '\t' << m_initial[e][0].x() << '\t' << fmod(m_initial[e][0].x(), 2. * PI) << '\n';
    // }
...
std::vector<Composer::IntersectionNode*> Composer::intersect(std::vector<Vec2> const& up, std::vector<Vec2> const& down)
{

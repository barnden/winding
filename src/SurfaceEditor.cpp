...
void SurfaceEditor::step(double dt, double ksp, double kdp, double eps)
{
    ...
    auto path = composer.simulate(dt, ksp, kdp, eps, m_step + 1);
    ...
    auto off_surface = std::vector<std::pair<Vec2, Vec3>> {};
    {
        ...
        for (auto&& [i, frame] : enumerate(path)) {
            ...
            Vec3 shadow_point = m_surface.f(frame.shadow);
            ostream << i << ", " << frame.position.x() << ", " << frame.position.y() << ", " << frame.position.z() << ", " << frame.distance << ", " << frame.path_distance << ", " << frame.angle << ", " << shadow_point.x() << ", " << shadow_point.y() << ", " << shadow_point.z() << '\n';
        }
    }
    ...
    // copy-assignment operator implicitly deleted due to const& member; use placement-new to avoid copy issues
